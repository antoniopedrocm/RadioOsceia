      ...
      <div className="mx-auto max-w-7xl p-3">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-sm font-semibold">{current.titulo}</p><p className="text-xs text-muted-foreground">{current.programa} • {current.tipo}</p></div>
          </div>
          <div className="flex items-center gap-1">
            <Button size="sm" variant="ghost" onClick={() => setPlaying(!playing)}>{playing ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}</Button>
        ...
        <Progress value={42} />
        {expanded && <div className="mt-2 flex items-center justify-between text-xs text-muted-foreground"><span className="inline-flex items-center gap-1"><Radio className="h-3 w-3 text-destructive" />Ao vivo agora</span><span>Origem: {current.tipo}</span></div>}
      </div>
    </motion.div>
  );
}
